...
# Read YAML file
yaml_path = os.path.join(os.path.dirname(SCRIPT_DIR), "configs", "config_mysql.yml")
...
send_slack_webhook(f"✅ *MYSQL Job Started*")
...
for job in config['jobs']:
    ...
    jdbc_url = "jdbc:mysql://"+db_server+":3306/"+database
    ...
    for i in range(0,n):  #change this for historical load
        ...
        query = f"(SELECT * FROM {table_name} WHERE DATE({incremental_date}) >= '{formatted_date}' AND DATE({incremental_date}) < '{formatted_next_day}') AS tmp"
...
send_slack_webhook(f"✅ *MYSQL Job Completed*")